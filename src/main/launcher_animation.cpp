...
#define TINY_STAR_COUNT 6
#define SMALL_STAR_COUNT 6
#define MEDIUM_STAR_COUNT 4
#define LARGE_STAR_COUNT 4

struct LauncherContext {
    AnimatedSvg logo_svg;
    AnimatedSvg recompiled_svg;
    AnimatedSvg sky_svg;
    //std::array<AnimatedSvg, TINY_STAR_COUNT> tiny_stars_svgs;
    //std::array<AnimatedSvg, SMALL_STAR_COUNT> small_stars_svgs;
    //std::array<AnimatedSvg, MEDIUM_STAR_COUNT> medium_stars_svgs;
    //std::array<AnimatedSvg, LARGE_STAR_COUNT> large_stars_svgs;
...
void banjo::launcher_animation_setup(recompui::LauncherMenu *menu) {
    ...
    background_container->set_background_color({ 0x21, 0x22, 0x27, 0xFF });
    ...
    for (auto option : menu->get_game_options_menu()->get_options()) {
        option->set_font_family("Arial Rounded MT Bold");
    ...
    launcher_context.sky_svg = create_animated_svg(context, launcher_context.wrapper, "BM64TSASky.svg", 7680.0f, 4320.0f);
    launcher_context.logo_svg = create_animated_svg(context, launcher_context.wrapper, "BM64TSALogo.svg", 400.0f * 2.5f, 222.0f * 2.5f);
    launcher_context.recompiled_svg = create_animated_svg(context, launcher_context.wrapper, "TSARecompiled.svg", 400.0f * 1.0f, 205.0f * 1.0f);

    // lower the logo and text by 40
    ...
    launcher_context.logo_svg.position_animation.interpolation_method = InterpolationMethod::Linear;
    ...
    launcher_context.recompiled_svg.position_animation.interpolation_method = InterpolationMethod::Linear;
