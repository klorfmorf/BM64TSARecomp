...
set_target_properties(BM64TSARecompiled PROPERTIES
        MACOSX_BUNDLE TRUE
        MACOSX_BUNDLE_BUNDLE_NAME "BM64TSARecompiled"
        MACOSX_BUNDLE_GUI_IDENTIFIER "com.github.bm64tsarecompiled"
...
target_sources(BM64TSARecompiled PRIVATE ${ICNS_FILE})

# Ensure BM64TSARecompiled depends on create_icns
add_dependencies(BM64TSARecompiled create_icns)

# Configure Info.plist
configure_file(${CMAKE_SOURCE_DIR}/.github/macos/Info.plist.in ${CMAKE_BINARY_DIR}/Info.plist @ONLY)

# Install the app bundle
install(TARGETS BM64TSARecompiled BUNDLE DESTINATION .)
...
add_custom_command(TARGET BM64TSARecompiled POST_BUILD
    ...
    COMMAND ${CMAKE_COMMAND} -E copy_directory ${CMAKE_BINARY_DIR}/temp_assets $<TARGET_BUNDLE_DIR:BM64TSARecompiled>/Contents/Resources/assets
    COMMAND ${CMAKE_COMMAND} -E remove_directory ${CMAKE_BINARY_DIR}/temp_assets

    # Copy controller database
    COMMAND ${CMAKE_COMMAND} -E copy ${CMAKE_SOURCE_DIR}/recompcontrollerdb.txt $<TARGET_BUNDLE_DIR:BM64TSARecompiled>/Contents/Resources/

    # Set RPATH
    COMMAND install_name_tool -add_rpath "@executable_path/../Frameworks/" $<TARGET_BUNDLE_DIR:BM64TSARecompiled>/Contents/MacOS/BM64TSARecompiled

    # Sign the bundle
    COMMAND codesign --verbose=4 --options=runtime --no-strict --sign - --entitlements ${ENTITLEMENTS_FILE} --deep --force $<TARGET_BUNDLE_DIR:BM64TSARecompiled>
